#!/usr/bin/env python3
"""
bulletin-board-github.py — Brain inter-agent task relay.

Runs on GitHub Actions every 6 hours.
Checks Cosmos DB for pending tasks targeted at brain/all.
...
Env vars required:
  ...
  GITHUB_REPOSITORY — owner/repo (e.g. Copilotclaw/brainstorm-local)
"""
...
GITHUB_REPO  = os.environ.get("GITHUB_REPOSITORY", "Copilotclaw/brainstorm-local")
AGENT_NAME   = "brain"
...
def main():
    ...
    for task in tasks:
        ...
        issue_body = f"""## 📋 Brainstorm Task from Bulletin Board
